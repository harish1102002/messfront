...
function Items(props) {
  ...
  const submit = (e) => {
    ...
    if (e != 0)
      for (let i = 3; i < props.post.length + 3; i++)
        if (props.post[i - 3].avl_qt < Number(input[i].value)) {
          alert("Insufficient Resources!");
          return;
        }

    let propslice = props.post.slice();

    for (let i = 3; i < props.post.length + 3; i++) {
      let z = document.getElementsByTagName("input")[i].value.trim();
      if (z.trim() != "") {
        a.push([i - 3, Number(z)]);
        propslice.push([i - 2, props.post[i - 3].avl_qt - Number(z)]);
    ...
    for (let i = 0; i < a.length; i++)
      a1[i] = { name: props.post[a[i][0]].name, qty: a[i][1] };
